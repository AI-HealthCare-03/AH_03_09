...
// TODO: POST /api/v1/ocr/upload 연동 (현재 toast만)
// TODO: BE 리포트 API 생기면 mock 교체
  ...
  const handleDrop = useCallback((accepted: File[], rejected: FileRejection[]) => {
    if (rejected.length > 0) {
      ...
      if (first === "file-too-large") {
        ...
        toast.error("PNG·JPG·PDF 파일만 업로드할 수 있어요.");
      } else if (first === "too-many-files") {
        toast.error(`한 번에 최대 ${MAX_FILES}개까지 업로드할 수 있어요.`);
      } else {
        toast.error("일부 파일을 업로드할 수 없습니다.");
      }
    }
    if (accepted.length > 0) {
      toast.success(`${accepted.length}개 파일 업로드 준비 완료`, {
        description: accepted.map((f) => f.name).join(", "),
      });
    }
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card
          {...getRootProps()}
          className={`group relative overflow-hidden rounded-2xl border-2 border-dashed transition lg:col-span-3 ${
            isDragActive
        ...
        >
          ...
          <CardContent className="flex flex-col items-center justify-center gap-3 py-12 text-center">
            ...
            <Button type="button" onClick={open} size="lg" className="pointer-events-auto mt-2">
              파일 선택
            </Button>
          </CardContent>
        </Card>
