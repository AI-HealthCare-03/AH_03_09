...
### 6.2.1 소프트 삭제 및 30일 보관 정책 (REQ-OCR-009)

문서 삭제 요청 시 즉시 하드 삭제하지 않고 30일 유예 기간을 두는 이유:

- **법적 대응**: 분쟁 발생 시 사용자가 실제로 업로드·처리한 데이터를 증거로 보존
- **운영 실수 복구**: 삭제 직후 복구 요청 시 운영자가 DB에서 `is_active = True`로 되돌릴 여지 확보
- **개인정보보호법 준수**: "삭제 요청 즉시 파기"보다 "일정 기간 후 확실한 파기"가 법령상 더 안전한 패턴

30일 경과 후에는 `app/core/cleanup.py`의 24시간 주기 루프가 파일(로컬/S3)과 DB 레코드를 완전 파기합니다.
UI에는 복구 기능을 별도로 제공하지 않으며, 30일 이내 복구는 서버 직접 접근으로만 가능합니다.
...
| 검토 항목 | 판단 내용 |
|----------|----------|
| Clova OCR 내부 전처리 | Clova OCR은 자체 딥러닝 전처리 파이프라인을 포함. 이미 이진화·회전 보정을 내부적으로 수행 |
| 입력 분포 변화 위험 | Clova 모델이 학습한 이미지 분포(원본 의료 문서 스캔)와 달라질 경우 오히려 신뢰도 저하 가능 |
| 이중 전처리 효과 | 고품질 원본 스캔 이미지에 추가 이진화·노이즈 제거를 적용하면 디테일 손실 발생 |
| 유지보수 복잡도 | OpenCV native 의존성 추가 → Docker 이미지 크기 증가, 빌드 시간 증가 |
...
| 전처리 단계 | 구현 위치 | 효과 |
|-----------|----------|------|
| 노이즈 라인 제거 | `ocr_parser._clean_ocr_text()` | 영수증·행정정보 12개 패턴 제거 → GPT 토큰 절약 |
| ICD-10 코드 복원 | `ocr_parser._restore_icd10_periods()` | 표 셀 분리로 깨진 코드 복원 (`H0411 → H04.11`) |
| 약물명 정규화 | `ocr_task._normalize_drug_name()` | 단위 표기 통일 (`5mg → 5밀리그램`) 후 pg_trgm 매칭 |

이 3단계 파이프라인을 통해 GPT 파싱 정확도를 높이고 API 비용을 절감했습니다.
